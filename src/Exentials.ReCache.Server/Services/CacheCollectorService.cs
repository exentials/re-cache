...
public sealed class CacheCollectorService(
...
) : IHostedService, IDisposable
{
    ...
    private readonly ReCacheProvider _cacheProvider = cacheProvider;
    private readonly ILogger<CacheCollectorService> _logger = logger;
    ...
    private void CleanJob(object? state)
    {
        if (!_isCleaning && !_runningBackup)
        {
            ...
            _logger.LogInformation("{jobName} Started.", nameof(CleanJob));
            var count = _cacheProvider.CleanExpired();
            _logger.LogInformation("{jobName} cleaned {count} keys.", nameof(CleanJob), count);
